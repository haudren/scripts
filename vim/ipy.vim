...
@with_subchannel
def run_this_file_with_args(args=''):
    msg_id = send('%%run %s %s' % (run_flags, repr(vim.current.buffer.name),)+" "+args)
    print_prompt("In[]: %%run %s %s" % (run_flags, repr(vim.current.buffer.name))+" "+args,msg_id)
...
command! -complete=file -nargs=* Runpy :py run_this_file_with_args("<args>")
